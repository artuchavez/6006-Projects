...
def read_params(number):
    with open(str(number)+'.in','r') as fin:
        x_prelim = fin.readline().rstrip().split(':')
        for i in xrange(len(x_prelim)):
            temp = x_prelim[i].split(",")
            x_prelim[i] = [float(elm) for elm in temp]
        x = x_prelim
        y_prelim = fin.readline().rstrip().split(',')
        y = [float(i) for i in y_prelim]
        theta_prelim = fin.readline().rstrip().split(',')
        theta = [float(i) for i in theta_prelim]
        return x,y,theta

def read_output(number):
    with open(str(number)+".out",'r') as fin:
        check_theta = fin.readline().rstrip().split(",")
